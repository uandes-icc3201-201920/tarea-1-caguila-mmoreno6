...
void * socketThread(void *args)
{
  ...
  while ( (rc=read(newSocket,buf,sizeof(buf))) > 0) { 
    ...
    if (ex == 1){
		cout << "caga por el ex" << endl;
		pthread_exit(NULL);
	} 
	cout << "no caga por el ex" << endl;
	...
	//Value val = { segundoNumero.size(), segundoNumero };
	...
	//unsigned long ul;
	//ul = strtoul (primerNumero.c_str(), NULL, 0);
	...
	//db.insert(std::pair<unsigned long, Value>(ul, val));	
	// Imprimir lo que hemos agregado al mapa KV.
	
	//cout << "inserto y eso es lo que devuelve" << endl;
	//cout << db[ul].size << " " << db[ul].data << endl;
